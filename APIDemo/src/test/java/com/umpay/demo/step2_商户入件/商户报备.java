...
public class 商户报备 {
	...
	String url = EnvConfig.serverURL +"/entry-provider-plat-client/merchants/apply";


	@Test
	public void test个人商户报备() {
		...
		try {
			...
			reqMer.put("merchantName", "海底捞测试商户1");//商户简称	16	M	商户交易显示名称 
			...
			System.out.println("addMerInfoPerson输出请求结果:"+ result.toString());
	...
	public void test企业商户报备() {
		...
		try {
			...
			//发送post请求
			String result = HttpUtilClient.doPostJson(url, new JSONObject(), reqMap);
			System.out.println("addMerInfoEnterprise输出请求结果:"+ result.toString());
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail();
